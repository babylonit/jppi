...
class JoanaChatBot {
  ...
  addBotMessageWithAvatar(message) {
    const messagesContainer = document.getElementById('chat-messages');
    const messageDiv = document.createElement('div');
    messageDiv.className = 'flex items-start';
    messageDiv.innerHTML = `
      <div class="w-6 h-6 rounded-full flex items-center justify-center mr-2 flex-shrink-0 overflow-hidden">
  ...
  addBotMessage(message) {
    this.addBotMessageWithAvatar(message);
  }
